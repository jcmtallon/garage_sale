...
export const SelectBox = ({ options, selectedValud, ...props }: OwnProps) => {
  return (
    <select
      className={`text-xs md:text-sm rounded py-1.5 md:py-2 pl-2.5 md:pl-4 pr-9 border border-green-600 focus:border-gray-900 focus:shadow focus:ring-0 w-full`}
      value={selectedValud}
      {...props}
    >
      {options.map((opt) => (
